...
# Refresh tokens are single-use: each jti is recorded here the first (and
# only allowed) time it is presented to /auth/refresh.
...
def consume_refresh_token(payload: dict) -> None:
    """Mark a refresh token's jti as spent; reject if already spent.

    The check-and-add is atomic so two concurrent replays of the same token
    cannot both succeed.
    """
    jti = payload["jti"]
    with _refresh_lock:
