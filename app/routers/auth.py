...
def register(payload: RegisterRequest, db: Session = Depends(get_db)):
    ...
    if org is None:
        ...
        try:
            ...
            # Lost the race to create this org: someone else committed the same
            # name first. Join it as a member instead of erroring.
            db.rollback()
            org = db.query(Organization).filter(Organization.name == payload.org_name).first()
            role = "member"
    ...
    try:
        ...
        # Lost a concurrent race for the same (org_id, username) pair.
        db.rollback()
        raise AppError(409, "USERNAME_TAKEN", "Username already taken")
    return {
        "user_id": user.id,
