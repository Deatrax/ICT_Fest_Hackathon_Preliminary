...
# Serializes the read-check-then-write sections of booking creation and
# cancellation, which SQLite alone does not make atomic. Must never be held
# across a notifications.* call: those take their own locks, and nesting the
# two orderings is what deadlocks the service.
...
def create_booking(
    ...
    with _booking_lock:
        ...
        # Held under the lock so a concurrent cancel cannot decrement this
        # room's counters before they have been incremented.
...
def cancel_booking(
    ...
    with _booking_lock:
        # Re-read under the lock: a status observed before acquiring it may
        # already have been flipped by a concurrent cancel.
        booking = (
            db.query(Booking)
            .join(Room, Booking.room_id == Room.id)
            .filter(Booking.id == booking_id, Room.org_id == user.org_id)
        ...
        booking_id_out = booking.id
        room_id_out = booking.room_id
        start_date_out = booking.start_time.date().isoformat()
        refund_amount_cents = refund_entry.amount_cents

    cache.invalidate_report(user.org_id)
    cache.invalidate_availability(room_id_out, start_date_out)
